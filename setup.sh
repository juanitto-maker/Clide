...
install_signal_cli() {
    ...
    if [ "$PLATFORM" = "termux" ]; then
        ...
        sudo ln -sf "$INSTALL_DIR/signal-cli-${SIGNAL_VERSION}/bin/signal-cli" /usr/local/bin/signal-cli
    fi
...
main() {
    ...
    if [ "$NEED_PYTHON" = true ] || [ "$NEED_GIT" = true ] || [ "$NEED_NODE" = true ] || [ "$NEED_SIGNAL" = true ]; then
